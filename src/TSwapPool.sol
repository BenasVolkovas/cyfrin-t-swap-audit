...
contract TSwapPool is ERC20 {
    ...
    // @done @audit-i reodrder state variables
    ...
    function deposit(
        ...
        uint256 minimumLiquidityTokensToMint,
        uint256 maximumPoolTokensToDeposit,
        // @done @audit-v @medium nused param deadline. Use modifier
        ...
        returns (uint256 liquidityTokensToMint)
    {
        if (wethToDeposit < MINIMUM_WETH_LIQUIDITY) {
            // @done @audit-i MINIMUM_WETH_LIQUIDITY is constant, no need to emit it
        ...
        if (totalLiquidityTokenSupply() > 0) {
            ...
            // @done @audit-i it would be better if this was before the _addLiquidityMintAndTransfer call
    ...
    function _addLiquidityMintAndTransfer(
        ...
        // @invalid @audit-v @medium
        _mint(msg.sender, liquidityTokensToMint);
        // @done @audit-v LOW incorrect event params order
    ...
    {
        // @done @audit-i no magic numbers
        // @done @audit-v HIGH 10'000 should be 1k
        return
    ...
    function swapExactInput(
        ...
        public
        // @done @audit-g this should be external
        revertIfZero(inputAmount)
        revertIfDeadlinePassed(deadline)
        returns (
            // @done @audit-v @low impact (will always return 0)
            uint256 output
        )
    {
     ...
     */
    // @done @audit-i missing nat-specs param deadline
    // @done @audit-v no parameter for maximum input amount (max tokens to sell)
        ...
        returns (uint256 inputAmount)
    {
        ...
        // @done @audit-v HIGH no sllipage protection
    ...
    function sellPoolTokens(
        ...
        // @skipped @audit-v no swapExactOutput has wrong parameters. It should be weth amount
        // @done @audit-v @high Use swapExactInput instead
        return
            swapExactOutput(
                i_poolToken,
                i_wethToken,
    ...
    function _swap(
        ...
        swap_count++;
        // @done @audit-v breaks x * y = k invariant
        if (swap_count >= SWAP_COUNT_MAX) {
            swap_count = 0;
            // @done @audit-i no magic numbers
            // @done @audit-g we can save some gas if we transfer the output token only once at the end
